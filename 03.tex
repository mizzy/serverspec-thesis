...
提案手法に基づき実装した汎用コマンド実行フレームワークをSpecInfra\cite{specinfra}，制御テストフレームワークをserverspec\cite{serverspec}と名付けた．実装の結果としてどのようにテストコードが記述できるようになったかを，\figref{fig:test-files-with-serverspec}，\figref{fig:test-users-with-serverspec}，\figref{fig:test-groups-with-serverspec}，\figref{fig:test-packages-with-serverspec}，\figref{fig:test-services-with-serverspec}，\figref{fig:test-ports-with-serverspec}，\figref{fig:test-interfaces-with-serverspec}，\figref{fig:test-zfs-with-serverspec}に示す．
